...
| `--min-bitrate` / `--keep-lowbitrate` | 过滤掉**码率过低**的片源（4K≥30000 / 1080p≥10000 / 720p≥5000 kbps；**默认开启**，用 `--keep-lowbitrate` 关闭） |
| `--no-foreign-dub` / `--keep-foreign-dub` | 过滤掉**外配版**（第一条音轨是俄配/乌配/外语配音而非原声：西里尔文、AVO/MVO/DVO、ukr/Hurtom、Dubbed、TrueFrench/VFF 等；**默认开启**，用 `--keep-foreign-dub` 保留）。国语/国配等国内配音不受影响 |
...
| `--delay SEC` | 每部搜索之间的间隔秒数，缓解索引器高频抽风（默认 5，设 0 关闭） |
| `--min-seeders N` | 做种数下限（批量模式默认 5） |
| `--cn-first` / `--no-cn-first` | 每部片的候选里国内片源优先回填（默认开启） |
| `--no-remux` / `--remux` | 过滤掉原盘/Remux（默认开启，用 `--remux` 保留） |
| `--no-foreign-dub` / `--keep-foreign-dub` | 过滤掉外配版（俄配/乌配/外语配音；默认开启，用 `--keep-foreign-dub` 保留） |
...
1. **搜索关键词**：优先用「英文名 + 年份」（公共站命中率最高）。搜不到时依次回退到
   「仅英文名」→「中文名 + 年份」。**不限分类**——很多外语片/老片/国内标签的资源在
   公共索引器上分类标错或没分类，硬限定「电影(2000)」会把它们全漏掉（例如 Tangerine
   限定 2000 是 0 条、不限分类有 40+ 条）。相关性过滤已能筛掉无关结果，需要时再用
   `-c` 手动限定分类。
...
3.5 **码率过滤**（默认开启）：按分辨率设最低视频码率门槛
   （4K≥30000 / 1080p≥10000 / 720p≥5000 kbps）。Prowlarr 不提供码率和时长，
   脚本用 `码率≈体积×8÷时长` 估算——时长取 CSV 的 `duration` 列，缺失则用
   `--duration`（默认 100 分钟）。为避免误删，这是**偏宽松**的估算：用的是总码率
   （含音轨，略高于视频码率）；分辨率读不出或算不出码率的，改用文件体积判定——
   **大于 10GB 才保留**，否则丢弃。低于 720p（480p/SD）的片源**一律丢弃**。
   用 `--keep-lowbitrate` 关闭。
3.6 **外配过滤**（默认开启）：丢掉第一条音轨为外语配音（非原声）的片源——
   识别标题里的西里尔文、俄配组标记（AVO/MVO/DVO/LVO/Дубляж）、乌克兰站
   （ukr/Hurtom）、`Dubbed`/`Doblado`、法配（TrueFrench/VFF/VFQ）等。
   国语/国配等**国内配音不受影响**；VOSTFR（法字幕原声）、以及仅写原始语种
   （Persian/Korean/Japanese 等，通常就是原声）**不会被误杀**。用
   `--keep-foreign-dub` 关闭。
4. **回填**：每部片取前 `--top` 名，每个结果写入 4 列：
   `magnet_N`（标准磁力）、`quality_N`（如 `1080p/BluRay/x265`，另会带
   `国内` / `原盘` / `内封` / `中字` 等标签）、`seeders_N`（做种数）、
   `title_N`（原始发布标题）。默认已开启「国内优先 + 排除原盘 + 码率门槛 + 排除外配」，
   可用 `--no-cn-first` / `--remux` / `--keep-lowbitrate` / `--keep-foreign-dub`
   覆盖，`--no-hardsub` 进一步排除内嵌字幕。

> 注意：因为逐行都要请求 Prowlarr（还要解析 .torrent），且每部之间默认间隔 5 秒
> （`--delay`），批量处理**较慢**。片单大时建议先小批量试跑。
>
> **断点续跑（按内容）**：输出已存在时，重跑会**保留已找到磁力的行、只重搜空行**
> （`magnet_1` 为空的）。所以某次因索引器抽风留下的空洞，直接重跑同一命令即可自动补齐，
> 不必 `--restart`。全部重来才用 `--restart`。整张表在内存中维护，每补完一行就把
> 完整表原子写回（临时文件+瞬间替换），所以磁盘上的输出**任何时刻都是完整的**——
> 中途中断（Ctrl-C/崩溃）已搜到的行都在，绝不会出现空文件，重跑接着补空行。
